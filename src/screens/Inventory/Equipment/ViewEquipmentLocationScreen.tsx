...
export default function ViewEquipmentLocationScreen() {
  ...
  const filteredEquipment = useMemo(() => {
    ...
    return equipment.filter(item => {
      ...
      if (item.model?.toLowerCase().includes(searchLower)) return true;
      return false;
    });
  ...
  const fetchLocationData = async () => {
    try {
      ...
      const locationResponse = await equipmentLocationApi.show(Number(locationId));
      const locationData = (locationResponse as any).data || locationResponse;
      setLocation(locationData);

      // Fetch equipment for this location
      const equipmentResponse = await equipmentApi.getByLocation(Number(locationId));
    ...
    <View style={styles.container}>
      ...
      <View style={styles.pageTitleContainer}>
        <View style={styles.titleLeftGroup}>
          ...
          >
            <Ionicons name="arrow-back" size={24} color="#0D253F" />
          </TouchableOpacity>
      ...
      >
        ...
        <View style={styles.actionButtonsRow}>
          <Button
            title="Add Equipment"
            onPress={() => router.push({ 
              pathname: '/inventory/equipment/add',
        ...
        {loading ? (
          ...
          <View style={styles.loadingContainer}>
            <Text style={styles.loadingText}>
              {searchText.trim() ? 'No equipment found matching your search.' : 'No equipment in this location.'}
            </Text>
          </View>
        ) : (
