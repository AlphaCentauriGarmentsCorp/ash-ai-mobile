...
export default function AddServiceTypeScreen() {
  ...
  const handleInputChange = (field: keyof CreateServiceTypeRequest, value: string) => {
    setFormData((prev: CreateServiceTypeRequest) => ({
      ...prev,
      [field]: value
    }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      const response = await serviceTypeApi.store(formData);
      ...
      const message = response.message || 'Service type created successfully!';
      ...
      if (isSuccess) {
        ...
        Alert.alert('Error', message || 'Failed to create service type');
      }
    } catch (error: any) {
      ...
      if (error.response?.status === 422 && error.response?.data?.errors) {
        ...
        Alert.alert('Error', error.message || 'Failed to create service type. Please try again.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.pageTitleContainer}>
        <View style={styles.titleLeftGroup}>
          ...
          <Text style={styles.pageTitleText}>Add Service Type</Text>
        </View>

        <View style={styles.breadcrumbGroup}>
          <Text style={styles.breadcrumbBold}>Home</Text>
          <Text style={styles.breadcrumbNormal}> / Dropdown Settings / Service Type / Add</Text>
        </View>
      </View>
