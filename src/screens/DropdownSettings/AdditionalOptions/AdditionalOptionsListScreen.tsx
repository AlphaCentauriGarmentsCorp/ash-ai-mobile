...
export default function AdditionalOptionsListScreen() {
  ...
  const fetchAdditionalOptions = useCallback(async (page: number = 1, search: string = '') => {
    try {
      setLoading(true);
      const response = await additionalOptionsApi.index({
        page,
      ...
      Alert.alert('Error', 'Failed to load additional options. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [entriesPerPage]);
  ...
  const handleRefresh = useCallback(async () => {
    setRefreshing(true);
    await fetchAdditionalOptions(1, searchText);
    setRefreshing(false);
  }, [fetchAdditionalOptions, searchText]);

  const handleSearch = useCallback((text: string) => {
    setSearchText(text);
    setCurrentPage(1);
    fetchAdditionalOptions(1, text);
  }, [fetchAdditionalOptions]);

  const handlePageChange = useCallback((page: number) => {
    if (page >= 1 && page <= totalPages) {
      setCurrentPage(page);
      fetchAdditionalOptions(page, searchText);
    }
  }, [totalPages, searchText, fetchAdditionalOptions]);

  const handleEntriesChange = useCallback((value: number) => {
    setEntriesPerPage(value);
    setCurrentPage(1);
    fetchAdditionalOptions(1, searchText);
  }, [searchText, fetchAdditionalOptions]);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await additionalOptionsApi.delete(id);
  ...
  const columns: Column[] = useMemo(() => [
    ...
    { key: 'description', header: 'Description', width: 300, sortable: true },
    {
      ...
      width: 100,
      sortable: false,
      render: (_value: any, item: AdditionalOption) => (
        <View style={styles.actionButtons}>
          <TouchableOpacity 
            style={styles.actionBtn}
            onPress={() => router.push(`/dropdown-settings/additional-options/edit/${item.id}` as any)}
          >
      ...
      <View style={styles.pageTitleContainer}>
        <View style={styles.titleLeftGroup}>
          <View style={styles.iconCircleWrapper}>
            <Ionicons name="options-outline" size={24} color="#0D253F" />
          </View>
          <Text style={styles.pageTitleText}>Additional Options</Text>
        </View>

        <View style={styles.breadcrumbGroup}>
          <Text style={styles.breadcrumbBold}>Home</Text>
          <Text style={styles.breadcrumbNormal}> / Dropdown Settings / Additional Options</Text>
        </View>
      </View>
      ...
      >
        <View style={styles.actionButtonsRow}>
          <Button
            ...
            onPress={() => router.push('/dropdown-settings/additional-options/add' as any)}
            variant="primary"
            size="base"
