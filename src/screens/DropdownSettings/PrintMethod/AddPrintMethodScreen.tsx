...
export default function AddPrintMethodScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const fontsLoaded = usePoppinsFonts();

  const [formData, setFormData] = useState<CreatePrintMethodRequest>({
    name: '',
    description: '',
  });
  const [loading, setLoading] = useState(false);

  const handleInputChange = (field: keyof CreatePrintMethodRequest, value: string) => {
    setFormData((prev: CreatePrintMethodRequest) => ({
      ...prev,
      [field]: value
    }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      const response = await printMethodApi.store(formData);
      
      console.log('API Response:', response);
      
      // Handle different response structures
      const isSuccess = response.success !== undefined ? response.success : true;
      const message = response.message || 'Print method created successfully!';
      ...
      if (isSuccess) {
        ...
        Alert.alert('Error', message || 'Failed to create print method');
      }
    } catch (error: any) {
      ...
      if (error.response?.status === 422 && error.response?.data?.errors) {
        ...
        Alert.alert('Error', error.message || 'Failed to create print method. Please try again.');
      }
    } finally {
      setLoading(false);
    }
