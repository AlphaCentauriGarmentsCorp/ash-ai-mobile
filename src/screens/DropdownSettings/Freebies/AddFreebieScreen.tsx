...
export default function AddFreebieScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const fontsLoaded = usePoppinsFonts();
  const { isAuthenticated } = useAuth();

  const [formData, setFormData] = useState<CreateFreebieRequest>({
    name: '',
    description: '',
  });
  const [loading, setLoading] = useState(false);

  const handleInputChange = (field: keyof CreateFreebieRequest, value: string) => {
    setFormData((prev: CreateFreebieRequest) => ({
      ...prev,
      [field]: value
    }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      const response = await freebiesApi.store(formData);
      ...
      const message = response.message || 'Freebie created successfully!';
      ...
      if (isSuccess) {
        ...
        Alert.alert('Error', message || 'Failed to create freebie');
      }
    } catch (error: any) {
      ...
      if (error.response?.status === 422 && error.response?.data?.errors) {
        ...
        Alert.alert('Error', error.message || 'Failed to create freebie. Please try again.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.pageTitleContainer}>
        <View style={styles.titleLeftGroup}>
          ...
          <Text style={styles.pageTitleText}>Add Freebie</Text>
        </View>

        <View style={styles.breadcrumbGroup}>
          <Text style={styles.breadcrumbBold}>Home</Text>
          <Text style={styles.breadcrumbNormal}> / Dropdown Settings / Freebies / Add</Text>
        </View>
      </View>
